...
const Invoice = () => {
    ...
    useEffect(() => {
        const getReceipt = async() => {
          const last_response = await fetch(`https://aaayotech.pythonanywhere.com/last_payment`, {
              headers: {
          ...
          if (last_response.status == 200) {
              ...
              //do something(`make i check: ${reference_id}`)
                const response = await fetch(`https://aaayotech.pythonanywhere.com/make_payment/?ref_id=${data['data']}`, {
                    headers: {
                        'Authorization': 'Bearer ' + token,
                        'user': username,
                        'Content-Type': 'application/json',
